...
use bevy_rapier2d::{
    ...
    prelude::{Collider, RigidBody, RigidBodyDisabled, TriMeshFlags, VHACDParameters},
};
...
fn update_collider(
    ...
    let trimesh = Collider::trimesh_with_flags(collider_pts, idx_buffer, TriMeshFlags::empty());

    let ball = Collider::ball((celestial.terrain.offset - celestial.terrain.multiplier) as f32);

    *celestial.collider = match trimesh {
        Ok(trimesh) => Collider::compound(vec![
            (Vec2::ZERO, 0.0, trimesh),
            (rigid_pos.as_vec2(), 0.0, ball),
        ]),
        Err(e) => {
            error!("Error building terrain tri mesh: {e}");
            Collider::compound(vec![(rigid_pos.as_vec2(), 0.0, ball)])
        }
    };
}
