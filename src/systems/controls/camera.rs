use crate::{
    ...
    consts::keybinds::{
        KB_CAM_FAST_MOD, KB_CAM_ROT_LEFT, KB_CAM_ROT_RIGHT, KB_CAM_SLOW_MOD, KB_CAM_ZOOM_IN,
        KB_CAM_ZOOM_OUT, KB_CAM_ZOOM_RESET,
    },
};
...
pub fn control_camera(
    ...
    if key.pressed(KB_CAM_ROT_LEFT) {
        camera.transform.rotate_z((delta_amount * TAU) as f32);
    }
    if key.pressed(KB_CAM_ROT_RIGHT) {
        camera.transform.rotate_z((-delta_amount * TAU) as f32);
    }
