...
impl LodVectors {
    ...
    fn update_lod(
        ...
        if level_not_loaded || lod_needs_updating {
            let vecs = terrain_gen.gen_lod(level.get(), new_focus);

            if level_not_loaded {
                self.0.push(vecs);
            } else {
                *self.0.get_mut(level.get() as usize).unwrap() = vecs;
            }
        }
    }
    ...
    fn create_unshifted_vertex_buffer(
        ...
        for level in (1..max_level).rev() {
            ...
            if next_start + 1 < LOD_VERTS as usize {
                vertices.extend_from_slice(&verts[next_start + SKIP_VERTS_AMOUNT..]);
            }
        }
    ...
    fn test_updating() {
        ...
        for i in 0..ITERS {
            ...
            if full_buffers.vertices != lazy_buffers.vertices {
                ...
                assert_eq!(full_buffers.vertices, lazy_buffers.vertices);
            }
